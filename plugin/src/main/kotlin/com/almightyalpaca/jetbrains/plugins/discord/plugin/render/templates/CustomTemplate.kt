...
object Utils {
    ...
    private fun sizeAsString(size: Int?): String? {
        ...
        if (size < 2 shl 20) return "${twoDecimals(size.toFloat() / (1 shl 10))} KiB" // 2 KiB .. 2 MiB
        if (size < 1 shl 30) return "${twoDecimals(size.toFloat() / (1 shl 20))} MiB" // 2 MiB .. 1 GiB
        return "${twoDecimals(size.toFloat() / (1 shl 30))} GiB" // 1 GiB ..
    }

    private fun twoDecimals(num: Float): Float = Math.floor(num * 100) / 100
