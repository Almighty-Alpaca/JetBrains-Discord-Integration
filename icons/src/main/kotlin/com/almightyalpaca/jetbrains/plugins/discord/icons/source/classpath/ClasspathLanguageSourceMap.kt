...
class ClasspathLanguageSourceMap(private val source: ClasspathSource, map: Map<String, LanguageSource>) : AbstractLanguageSourceMap(map) {
    override fun createLanguageMap(languages: Map<String, Language>): LanguageMap = ClasspathLanguageMap(languages.values)

    override fun createDefaultLanguage(name: String, assetId: String): Language.Default = ClasspathLanguage.Default(source, name, assetId)
    ...
    override fun createSimpleLanguage(fileId: String, name: String, parent: Language?, assetIds: List<String>?, matchers: Map<Matcher.Target, Matcher>): Language.Simple =
        ClasspathLanguage.Simple(source, fileId, name, parent, assetIds, matchers)
}
