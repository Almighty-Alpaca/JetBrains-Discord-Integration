...
JNIEXPORT jobject JNICALL Java_com_almightyalpaca_jetbrains_plugins_discord_gamesdk_impl_DiscordUserManagerImpl_native_1getCurrentUser
    ...
    namespace JDiscordUser = gamesdk::api::types::DiscordUser;
    jobject j_user = nullptr;
    if (result == DiscordResult_Ok)
        j_user = JDiscordUser::constructor0::invoke(*env, user.id, env->NewStringUTF(user.username), env->NewStringUTF(user.discriminator), env->NewStringUTF(user.avatar), user.bot);
    return types::createPair(*env, types::createIntegerObject(*env, result), j_user);
}
...
JNIEXPORT jobject JNICALL Java_com_almightyalpaca_jetbrains_plugins_discord_gamesdk_impl_DiscordUserManagerImpl_native_1getCurrentUserPremiumType
    ...
    return types::createPair(*env, types::createIntegerObject(*env, (jint) result), types::createIntegerObject(*env, premium_type));
}
...
JNIEXPORT jobject JNICALL Java_com_almightyalpaca_jetbrains_plugins_discord_gamesdk_impl_DiscordUserManagerImpl_native_1currentUserHasFlag
    ...
    return types::createPair(*env, types::createIntegerObject(*env, (jint) result), types::createBooleanObject(*env, has_flag));
}
