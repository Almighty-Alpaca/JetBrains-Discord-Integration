...
int RelationshipSnowflakeFilter(void *data, struct DiscordRelationship *relationship) {
    auto *app = (struct Application *) data;
...
void OnRelationshipsRefresh(void *data) {
    ...
    module->filter(module, app, (bool (*)(void *, struct DiscordRelationship *)) RelationshipPassFilter);

    int32_t unfiltered_count = 0;
    DISCORD_REQUIRE(module->count(module, &unfiltered_count));

    module->filter(module, app, (bool (*)(void *, struct DiscordRelationship *)) RelationshipSnowflakeFilter);
